...
bool nvMemory::saveConfig(TSettings* Settings)
{
    if (init())
    {
        ...
        DEBUG_SERIAL_PRINTLN(F("SPIFS: Saving configuration."));
        ...
        if (!configFile)
        {
            ...
            DEBUG_SERIAL_PRINTLN("SPIFS: Failed to open config file for writing");
            return false;
        }
        ...
        {
            // Error writing file
            DEBUG_SERIAL_PRINTLN(F("SPIFS: Failed to write to file"));
            return false;
        }
        // Close file
        configFile.close();
...
bool nvMemory::loadConfig(TSettings* Settings)
{
    ...
    if (init())
    {
        if (SPIFFS.exists(JSON_CONFIG_FILE))
        {
            ...
            if (configFile)
            {
                DEBUG_SERIAL_PRINTLN("SPIFS: Loading config file");
                StaticJsonDocument<512> json;
                ...
                else
                {
                    ...
                    DEBUG_SERIAL_PRINTLN("SPIFS: Error parsing config file!");
                }
            }
            else
            {
                DEBUG_SERIAL_PRINTLN("SPIFS: Error opening config file!");
            }
        }
        else
        {
            DEBUG_SERIAL_PRINTLN("SPIFS: No config file available!");
        }
    }
    return false;
}
...
{
    DEBUG_SERIAL_PRINTLN("SPIFS: Erasing config file..");
...
bool nvMemory::init()
{
    if (!Initialized_)
    {
        DEBUG_SERIAL_PRINTLN("SPIFS: Mounting File System...");
        // May need to make it begin(true) first time you are using SPIFFS
        Initialized_ = SPIFFS.begin(false) || SPIFFS.begin(true);
        Initialized_ ? DEBUG_SERIAL_PRINTLN("SPIFS: Mounted") : DEBUG_SERIAL_PRINTLN("SPIFS: Mounting failed.");
    }
    else
    {
        DEBUG_SERIAL_PRINTLN("SPIFS: Already Mounted");
    }
    return Initialized_;
};
